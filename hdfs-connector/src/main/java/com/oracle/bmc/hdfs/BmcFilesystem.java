...
public class BmcFilesystem extends FileSystem {
    ...
    private static synchronized void setupFilesystemCache(Configuration configuration) {
        ...
        /*
        Who ever had the BmcFilesystemImpl instance at the point of time it was removed from cache,
    ...
    public void close() throws IOException {
        ...
        if (delegate.ownersCount() == 0 ) {
            ...
            if(delegteFSKey != null && fsCache != null ) {
                /*
                Concurrency case :
                we can even remove a new value for delegteFSKey from fsCache , however the BMCFileSystem which
                had that instance of delegate(BMCFileSystemImpl) can continue to use it until they close them out. the new caller asking for the key will get to create a new one and add to cache
                However, not invalidating the cache from here can result in the problem that we have a closed out BMCFileSystemImpl for the key in cache and we give that to the caller
                 */
